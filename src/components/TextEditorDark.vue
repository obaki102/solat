<template>
  <div>
    <div class="flex flex-col max-w-container bg-slate-700 bg-clip-border rounded-xl p-4">
      <div v-if="editor"
      class="max-t-container buttons flex flex-wrap items-center gap-x-4 bg-slate-700 border border-slate-200 rounded-xl p-4 mb-2">
        <button @click="editor.chain().undo().run()" :disabled="!editor.can().chain().undo().run()"
          class="flex hover:bg-slate-200 p-1 rounded-lg">
          <svg xmlns="http://www.w3.org/2000/svg" class="fill-slate-900 hover:fill-slate-800" width="20" height="20" viewBox="0 0 24 24">
            <title>undo</title>
            <path
        ...
        <button @click="editor.chain().redo().run()" :disabled="!editor.can().chain().redo().run()"
          class="flex hover:bg-slate-100 p-1 rounded-lg">
          <svg xmlns="http://www.w3.org/2000/svg" class="fill-slate-900 hover:fill-slate-800" width="20" height="20" viewBox="0 0 24 24">
            <title>redo</title>
            <path
        ...
        <div class="h-6 border-l border-solid border-gray-700"></div>


        <button @click="editor.chain().toggleBold().run()" :disabled="!editor.can().chain().toggleBold().run()"
          class="flex hover:bg-slate-100 p-1 rounded-lg"
          :class="{ 'bg-slate-200 rounded ': editor.isActive('bold') }">
          <svg xmlns="http://www.w3.org/2000/svg" class="fill-slate-900 hover:fill-slate-800" 
            :class="{ 'fill-slate-800': editor.isActive('bold') }" width="20" height="20" viewBox="0 0 24 24">
            <title>bold</title>
            <path
        ...
        <button @click="editor.chain().toggleItalic().run()" :disabled="!editor.can().chain().toggleItalic().run()"
          class="flex hover:bg-slate-100 p-1 rounded-lg" :class="{ 'bg-slate-200 rounded': editor.isActive('italic') }">
          <svg xmlns="http://www.w3.org/2000/svg" class="fill-slate-900 hover:fill-slate-800" width="20" height="20" viewBox="0 0 24 24">
            <title>italic</title>
        ...
        <button @click="editor.chain().toggleStrike().run()" :disabled="!editor.can().chain().toggleStrike().run()"
          class="flex hover:bg-slate-100 p-1 rounded-lg" :class="{ 'bg-slate-200 rounded': editor.isActive('strike') }">
          <svg xmlns="http://www.w3.org/2000/svg" class="fill-slate-900 hover:fill-slate-800" width="20" height="20" viewBox="0 0 24 24">
            <title>strike</title>
            <path
        ...
        <button @click="editor.chain().toggleCode().run()" :disabled="!editor.can().chain().toggleCode().run()"
          class="flex hover:bg-slate-100 p-1 rounded-lg" :class="{ 'bg-slate-200 rounded': editor.isActive('code') }">
          <svg xmlns="http://www.w3.org/2000/svg" class="fill-slate-900 hover:fill-slate-800" width="20" height="20" viewBox="0 0 24 24">
            <title>code</title>
            <path
        ...
        <button @click="editor.chain().toggleHeading({ level: 1 }).run()" class="flex hover:bg-slate-100 p-1 rounded-lg"
          :class="{ 'bg-slate-200 rounded': editor.isActive('heading', { level: 1 }) }">
          <svg xmlns="http://www.w3.org/2000/svg" class="fill-slate-900 hover:fill-slate-800" width="20" height="20" viewBox="0 0 24 24">
            <title>H1</title>
            <path d="M3,4H5V10H9V4H11V18H9V12H5V18H3V4M14,18V16H16V6.31L13.5,7.75V5.44L16,4H18V16H20V18H14Z" />
          </svg>
        </button>
        <button @click="editor.chain().toggleHeading({ level: 2 }).run()" class="flex hover:bg-slate-100 p-1 rounded-lg"
          :class="{ 'bg-slate-200 rounded': editor.isActive('heading', { level: 2 }) }">
          <svg xmlns="http://www.w3.org/2000/svg" class="fill-slate-900 hover:fill-slate-800" width="20" height="20" viewBox="0 0 24 24">
            <title>H2</title>
            <path
        ...
        <button @click="editor.chain().toggleHeading({ level: 3 }).run()"  class="flex hover:bg-slate-100 p-1 rounded-lg"
          :class="{ 'bg-slate-200 rounded': editor.isActive('heading', { level: 3 }) }">
          <svg xmlns="http://www.w3.org/2000/svg" class="fill-slate-900 hover:fill-slate-800" width="20" height="20" viewBox="0 0 24 24">
            <title>H3</title>
            <path
        ...
        <button @click="editor.chain().toggleBulletList().run()" class="flex hover:bg-slate-100 p-1 rounded-lg"
          :class="{ 'bg-slate-200 rounded': editor.isActive('bulletList') }">
          <svg xmlns="http://www.w3.org/2000/svg" class="fill-slate-900 hover:fill-slate-800" width="20" height="20" viewBox="0 0 24 24">
            <title>bullet-list</title>
            <path d="M3,4H7V8H3V4M9,5V7H21V5H9M3,10H7V14H3V10M9,11V13H21V11H9M3,16H7V20H3V16M9,17V19H21V17H9" />
          </svg>
        </button>
        <button @click="editor.chain().toggleOrderedList().run()" class="flex hover:bg-slate-100 p-1 rounded-lg"
          :class="{ 'bg-slate-200 rounded': editor.isActive('orderedList') }">
          <svg xmlns="http://www.w3.org/2000/svg" class="fill-slate-900 hover:fill-slate-800" width="20" height="20" viewBox="0 0 24 24">
        ...
        <button @click="editor.chain().toggleCodeBlock().run()" class="flex hover:bg-slate-100 p-1 rounded-lg"
          :class="{ 'bg-slate-200 rounded': editor.isActive('codeBlock') }">
          <svg xmlns="http://www.w3.org/2000/svg" class="fill-slate-900 hover:fill-slate-800" width="20" height="20" viewBox="0 0 24 24">
        ...
        <button @click="editor.chain().toggleBlockquote().run()" class="flex hover:bg-slate-100 p-1 rounded-lg"
          :class="{ 'bg-slate-200 rounded': editor.isActive('blockquote') }">
          <svg xmlns="http://www.w3.org/2000/svg" class="fill-slate-900 hover:fill-slate-800" width="20" height="20" viewBox="0 0 24 24">
            <title>block-quote</title>
            <path d="M14,17H17L19,13V7H13V13H16M6,17H9L11,13V7H5V13H8L6,17Z" />
          </svg>
        </button>
        <button @click="editor.chain().setHorizontalRule().run()" class="flex hover:bg-slate-100 p-1 rounded-lg">
          <svg xmlns="http://www.w3.org/2000/svg" class="fill-slate-900 hover:fill-slate-800" width="20" height="20" viewBox="0 0 24 24">
            <title> horizontal rule</title>
            <path d="M21 11H3V9H21V11M21 13H3V15H21V13Z" />
          </svg>
        </button>
        <button @click="editor.chain().setHardBreak().run()" class="flex hover:bg-slate-100 p-1 rounded-lg">
          <svg xmlns="http://www.w3.org/2000/svg" class="fill-slate-900 hover:fill-slate-800" width="20" height="20" viewBox="0 0 24 24">
      ...
      <div class="max-w-container bg-slate-700 border border-slate-200 rounded-xl p-2">
...
.tiptap-dark .tiptap {
  ...
  /* Set the color to white for headings, bullet points, etc. */
  h1,
  h2,
  h3,
  h4,
