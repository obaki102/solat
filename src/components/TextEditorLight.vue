    ...
    <div class="flex flex-col max-w-container bg-white bg-clip-border rounded-xl p-4">
      <div v-if="editor"
        ...
        <button @click="editor.chain().toggleBold().run()" :disabled="!editor.can().chain().toggleBold().run()"
          class="flex hover:bg-gray-300 p-1 rounded-lg" :class="{ 'bg-gray-300 rounded': editor.isActive('bold') }">
          <svg xmlns="http://www.w3.org/2000/svg"  width="20" height="20" viewBox="0 0 24 24">
            <title>bold</title>
            <path
        ...
        <button @click="editor.chain().toggleHeading({ level: 3 }).run()" class="flex hover:bg-gray-300 p-1 rounded-lg"
